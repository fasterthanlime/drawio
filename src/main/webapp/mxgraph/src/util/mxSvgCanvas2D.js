...
mxSvgCanvas2D.prototype.createSvgGradient = function (
  ...
  var gradient = this.createElement(
    direction == mxConstants.DIRECTION_RADIAL
      ? "radialGradient"
      : "linearGradient",
  );
 ...
 */
mxSvgCanvas2D.prototype.createFillPatternId = function (
  type,
  strokeSize,
  color,
) {
 ...
 */
mxSvgCanvas2D.prototype.getFillPattern = function (
  type,
  strokeSize,
  color,
  scale,
...
mxSvgCanvas2D.prototype.createHatchPattern = function (
  strokeSize,
  color,
  scale,
) {
...
mxSvgCanvas2D.prototype.createDashedPattern = function (
  strokeSize,
  color,
  scale,
) {
...
mxSvgCanvas2D.prototype.createZigZagLinePattern = function (
  strokeSize,
  color,
  scale,
) {
...
mxSvgCanvas2D.prototype.createCrossHatchPattern = function (
  strokeSize,
  color,
  scale,
) {
...
mxSvgCanvas2D.prototype.createDotsPattern = function (
  strokeSize,
  color,
  scale,
) {
  ...
  if (s.scale != 1) {
    ...
    if (tok.length > 0) {
      ...
      if (args.length > 3) {
        function arg(index) {
          return (
            Math.round(parseFloat(args[index]) * s.scale * 100) / 100 + "px"
          );
        }
...
mxSvgCanvas2D.prototype.updateFill = function () {
  ...
  var pId =
    s.fillStyle == null || s.fillStyle == "auto" || s.fillStyle == "solid"
      ? null
      : this.getFillPattern(
          s.fillStyle,
          this.getCurrentStrokeWidth(),
          fill,
          s.scale,
        );
  ...
  try {
    ...
    if (bbox != null) {
      ...
      node.setAttribute(
        "x",
        bounds.x * s.scale -
          (bounds.width * s.scale * bbox.x) / bbox.width +
          this.imageOffset,
      );
      node.setAttribute(
        "y",
        bounds.y * s.scale -
          (bounds.height * s.scale * bbox.y) / bbox.height +
          this.imageOffset,
      );
...
mxSvgCanvas2D.prototype.addTextBackground = function (
  ...
  if (
    document.body != null &&
    (s.fontBackgroundColor != null || s.fontBorderColor != null)
  ) {
    var bbox = null;
